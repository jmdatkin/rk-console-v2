...
const props = defineProps(['assignment_data', 'driver_id', 'message']);
...
const submitDriverAssignment = function (day) {
    Inertia.visit(`/manage/driver/${props.driver_id}/assign/${selectedRoute.value.id}/${weekdaySelected.value}`,
        {
            method: 'post',
            onFinish: page => {
                getAssignmentData();
...
const getAssignmentData = function () {
    ...
    axios.get(`/manage/driver/${props.driver_id}/assignments`)
        .then(res => {
    ...
    <div>
        <Dialog v-model:visible="routeSelectOpen">
            ...
            <DataTable v-if="routeDataLoaded" :value="routeData" :paginator="true" :rows="10"
                ...
                @row-select="submitDriverAssignment(selectedRoute)">
                <template #header>
