...
const { data, dataLoaded, getData } = useData(route('report.meals.data', 'thu'));
    ...
    <ReportLayout>
        ...
        <template #report>
            ...
            <DataTable v-else :value="recipientData" :paginator="true" :rows="10" class="p-datatable-recipients"
                ...
                <Column :sortable="true" field="name" header="Route Name" >
                    <template #filter="{ filterModel, filterCallback }">
                        <InputText type="text" v-model="filterModel.value" @keydown.enter="filterCallback()"
                            class="p-column-filter" placeholder="Search by route name"></InputText>
                    </template>
                </Column>
                <Column :sortable="true" field="agg_num_meals" header="Total Num. Meals">
                    <!-- <template #body="{ data }">
                        {{ data.aggNumMeals }}
                    </template> -->
                    <template #filter="{ filterModel, filterCallback }">
